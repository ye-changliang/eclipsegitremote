...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_apis').bootstrapTable({
            url: EventInit.getCtx() + '/api-config/apis',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
var ButtonInit = function () {
    ...
    oInit.Init = function () {
        ...
        $('#btn_query').on('click', function () {
            $('#tb_apis').bootstrapTable('refresh', {query: TableInit.queryParams});
        });
        ...
        $('#btn_update').on('click', function () {
            var rows = $("#tb_apis").bootstrapTable('getSelections');
            if (rows.length == 0) {
                toastr.info("请选择一条记录");
                return false;
            }
        ...
        $('#myModal').find("#save").on('click', function () {
            ...
            $.ajax({
                type: "POST",
                url: EventInit.getCtx() + "/api-config/save",
                data: $from.serialize(),
        ...
        $('#delete-confirm').find('#delete-sure').on('click', function () {
            ...
            $.ajax({
                type: "DELETE",
                url: EventInit.getCtx() + "/api-config/delete",
                dataType: "json",
        ...
        $.ajax({
            type: "GET",
            datatype: "json",
            url: EventInit.getCtx() + '/api-kafka-config/findKafkaCodeList',
            success: function (data) {
                $('#myModal').find("#kafkaCode").append("<option  >请选择</option>");
                for (var i = 0; i < data.length; i++) {
                    $('#myModal').find("#kafkaCode").append("<option  value='" + data[i] + "'>" + data[i] + "</option>");
                }
                $('#myModal').find("#kafkaCode").selectpicker('refresh');
            }
        });
    };
