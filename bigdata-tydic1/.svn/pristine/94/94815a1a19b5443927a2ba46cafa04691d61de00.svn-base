...
var OnlineTableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_kafKaOnline').bootstrapTable({
            url:EventInit.getCtx()+ '/kafKa/queryKafKaOnlineFile',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
var ButtonInit1 = function () {
    ...
    $('#kafkaOnline').find("#hostServeList1").on('change', function () {
        //1.初始化Table
        var onlineOTable = new OnlineTableInit();
        onlineOTable.Init();
        $('#tb_kafKaOnline').bootstrapTable('refresh');
    });
    //模态框弹出事件
    $('#kafkaOnline').on('show.bs.modal', function (event) {
        $.ajax({
            type: "POST",
    ...
    //跟新到数据库事件
    $('#kafkaOnline').find("#save-import").on('click', function () {
        var rows = $("#tb_kafKaOnline").bootstrapTable('getSelections');
        if (rows.length == 0) {
            toastr.info("请选择一条记录");
            return false;
        }
        ...
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/kafKa/kafKaOnlineImport",
            dataType:"json",
