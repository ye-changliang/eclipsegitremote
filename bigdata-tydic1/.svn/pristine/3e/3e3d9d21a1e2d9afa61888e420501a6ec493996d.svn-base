...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_spark').bootstrapTable({
            url:EventInit.getCtx()+'/spark-serve/findSparkByIpAndAppNameIsNull',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
var ButtonInit = function () {
    ...
    function setModalValue(value) {
        if (value) {
            ...
            $modal.find("#myModalLabel").val("修改spark配置信息");
        } else {
            var $modal = $('#myModal');
            ...
            $modal.find("#myModalLabel").val("新增spark配置信息");
        }
        $.ajax({
            type: "POST",
    ...
    $('#myModal').find("#save").on('click', function () {
        ...
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/spark-serve/sparkSave",
            data: $from.serialize(),
            success: function (data) {
                if (data) {
                    toastr.success('保存成功！');
    ...
    function doMian(value) {
        ...
        {
            case "back":
                url= EventInit.getCtx()+"/spark-serve/backSparks";
                doWhat="备份";
                break;
        }
        $('#loadingModal').modal('show');
