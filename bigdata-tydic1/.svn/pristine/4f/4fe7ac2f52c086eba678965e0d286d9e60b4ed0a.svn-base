...
var OnlineTableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_flumeOnline').bootstrapTable({
            url: EventInit.getCtx()+'/flume/queryFlumeOnlineFile',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
var ButtonInit1 = function () {
    ...
    //模态框弹出事件
    $('#flumeOnline').on('show.bs.modal', function (event) {
        $.ajax({
            type: "POST",
    ...
    $('#flumeOnline').find("#hostServeList1").on('change', function () {
        //1.初始化Table
        var onlineOTable = new OnlineTableInit();
        onlineOTable.Init();
        $('#tb_flumeOnline').bootstrapTable('refresh');
    });

    //跟新到数据库事件
    $('#flumeOnline').find("#save-import").on('click', function () {
        var rows = $("#tb_flumeOnline").bootstrapTable('getSelections');
        if (rows.length == 0) {
            toastr.info("请选择一条记录");
            return false;
        }
        ...
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/flume/flumeOnlineImport",
            dataType:"json",
