...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_kafKa').bootstrapTable({
            url: EventInit.getCtx()+'/kafKa/findKafKaByIp',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            ...
            columns: [{
                ...
                field: 'kafkaConfigPath',
                title: '配置文件路径'
            }, {
                field: 'kafkaConfigDesc',
                title: '配置文件描述'
            }, {
                field: 'updateDate',
                title: '更新时间'
...
var ButtonInit = function () {
    ...
    oInit.Init = function () {
        $('#btn_query').on('click',function () {
            $('#tb_kafKa').bootstrapTable('refresh', {query: TableInit.queryParams});
        });
    ...
    function setModalValue(value) {
        if (value) {
            var $modal = $('#myModal');
            $modal.find("#kafkaId").val(value.kafkaId);
            $modal.find("#hostIp").val(value.ip);
            $modal.find("#kafkaConfig").val(value.kafkaConfig);
            $modal.find("#kafkaConfigDesc").val(value.kafkaConfigDesc);
            $modal.find("#kafkaConfigContent").val(value.kafkaConfigContent);
            $modal.find("#myModalLabel").html("修改kafKa配置信息")
        } else {
            var $modal = $('#myModal');
            $modal.find("#kafkaId").val("");
            $modal.find("#hostIp").val("");
            $modal.find("#kafkaConfig").val("");
            $modal.find("#kafkaConfigDesc").val("");
            $modal.find("#kafkaConfigContent").val("");
            $modal.find("#myModalLabel").html("新增kafKa配置信息")
        }
        $.ajax({
            type: "POST",
            url:EventInit.getCtx()+ "/host-serve/findAllHostServeListByServeName",
            data: {hostServeName:"kafka"},
    ...
    $('#myModal').find("#save").on('click', function () {
        debugger
        $from = $('#myModal').find('#kafKaInfo');
        $.ajax({
            type: "POST",
            url:EventInit.getCtx()+"/kafKa/kafKaSave",
            data: $from.serialize(),
            success: function (data) {
                if (data) {
                    toastr.success('保存成功！');
                ...
                $('#myModal').modal('hide');
                $('#tb_kafKa').bootstrapTable('refresh');
            },
            error: function () {
                toastr.error('服务器异常！');
            }
    ...
    $('#myModal').find("#testHostConnet").on('click', function () {
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/kafKa/testHostConnet",
    ...
    $('#btn_update').on('click', function () {
        var rows = $("#tb_kafKa").bootstrapTable('getSelections');
        if (rows.length == 0) {
            toastr.info("请选择一条记录");
            return false;
        }
    ...
    $('#delete-confirm').find('#delete-sure').on('click', function () {
        ...
        $.ajax({
            type: "DELETE",
            url:EventInit.getCtx()+ "/kafKa/batchDeleteKafKas",
            dataType:"json",
    ...
    $('#myDeletModal').find('#delete').on('click', function () {
        ...
        $.ajax({
            type: "DELETE",
            url:EventInit.getCtx()+ "/kafKa/batchDeleteKafKasData",
            dataType:"json",
            contentType:"application/json", // 指定这个协议很重要
            data: JSON.stringify(rows),
            success: function (data) {
    ...
    ///////////////停止、重启、备份确定事件//////////////////////////////////////////
    $('#kafKaModal').find('#sure').on('click', function () {
        var $modal = $('#kafKaModal');
    ...
    function doMian(value) {
        if(!value)return
        var rows = $("#tb_kafKa").bootstrapTable('getSelections');
        var kafkaId=rows[0].kafkaId;
        var url="";
        var doWhat;
        switch(value)
        {
            case "back":
                url=EventInit.getCtx()+"/kafKa/backKafKas";
                doWhat="备份";
                break;
            case "reset":
                url=EventInit.getCtx()+"/kafKa/resetKafKas";
                doWhat="重启";
                break;
            case "stop":
                url=EventInit.getCtx()+"/kafKa/stopKafKas";
                doWhat="停止";
                break;
        }
        $('#loadingModal').modal('show');
