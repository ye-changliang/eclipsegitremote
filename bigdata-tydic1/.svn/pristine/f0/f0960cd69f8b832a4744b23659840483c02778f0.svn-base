...
var OnlineTableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_redisOnline').bootstrapTable({
            url: EventInit.getCtx()+'/redis/queryRedisOnlineFile',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
var ButtonInit1 = function () {
    ...
    $('#redisOnline').find("#hostServeList1").on('change', function () {
        //1.初始化Table
        var onlineOTable = new OnlineTableInit();
        onlineOTable.Init();
        $('#tb_redisOnline').bootstrapTable('refresh');
    });

    //模态框弹出事件
    $('#redisOnline').on('show.bs.modal', function (event) {
        $.ajax({
            type: "POST",
    ...
    //跟新到数据库事件
    $('#redisOnline').find("#save-import").on('click', function () {
        var rows = $("#tb_redisOnline").bootstrapTable('getSelections');
        if (rows.length == 0) {
            toastr.info("请选择一条记录");
            return false;
        }
        ...
        $.ajax({
            type: "POST",
            url:EventInit.getCtx()+ "/redis/redisOnlineImport",
            dataType:"json",
