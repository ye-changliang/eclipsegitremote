...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_flume').bootstrapTable({
            url:EventInit.getCtx()+ '/flume/findFlumeByIp',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            ...
            columns: [{
                ...
                field: 'flumeConfigPath',
                title: '配置文件路径'
            }, {
                field: 'flumeConfigDesc',
                title: '配置文件描述'
            }, {
                field: 'updateDate',
                title: '更新时间'
    ...
    oTableInit.queryParams = function (params) {
        var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            ip: $("#ip").val(),
            configType:"flume",
            flumeConfig:$("#flumeConfigName").val(),
        };
        return temp;
    };
    ...
    $("#flumeConfig").bind("onblur",function () {
        var fileName=$("#flumeConfig").val();
        if(fileName=='')
            return
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/flume/checkFlumesByConfigName",
...
var ButtonInit = function () {
    ...
    oInit.Init = function () {
        $('#btn_query').on('click',function () {
            $('#tb_flume').bootstrapTable('refresh', {query: TableInit.queryParams});
        });
    ...
    function setModalValue(value) {
        if (value) {
            var $modal = $('#myModal');
            $modal.find("#flumeId").val(value.flumeId);
            $modal.find("#hostIp").val(value.ip);
            $modal.find("#appName").val(value.appName);
            $modal.find("#flumeConfig").val(value.flumeConfig);
            $modal.find("#flumeConfigDesc").val(value.flumeConfigDesc);
            $modal.find("#flumeConfigContent").val(value.flumeConfigContent);
            $modal.find("#myModalLabel").html("修改flume配置信息")
        } else {
            var $modal = $('#myModal');
            $modal.find("#flumeId").val("");
            $modal.find("#hostIp").val("");
            $modal.find("#appName").val("");
            $modal.find("#flumeConfig").val("");
            $modal.find("#flumeConfigDesc").val("");
            $modal.find("#flumeConfigContent").val("");
            $modal.find("#myModalLabel").html("新增flume配置信息")
        }
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/host-serve/findAllHostServeListByServeName",
            data: {hostServeName:"flume"},
            success: function (data) {
                if (data) {
    ...
    $('#myModal').find("#save").on('click', function () {
        $from = $('#myModal').find('#flumeInfo');
        $('#loadingModal').modal('show');
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/flume/flumeSave",
            data: $from.serialize(),
            success: function (data) {
                if (data) {
                    toastr.success('保存成功！');
                ...
                $('#myModal').modal('hide');
                $('#tb_flume').bootstrapTable('refresh');
    ...
    $('#myModal').find("#testHostConnet").on('click', function () {
        $.ajax({
            type: "POST",
            url: EventInit.getCtx()+"/flume/testHostConnet",
    ...
    $('#btn_update').on('click', function () {
        var rows = $("#tb_flume").bootstrapTable('getSelections');
        if (rows.length == 0) {
            toastr.info("请选择一条记录");
            return false;
        }
    ...
    $('#delete-confirm').find('#delete-sure').on('click', function () {
        ...
        $.ajax({
            type: "DELETE",
            url: EventInit.getCtx()+"/flume/batchDeleteFlumes",
            dataType:"json",
    ...
    $('#myDeletModal').find('#delete').on('click', function () {
        ...
        $.ajax({
            type: "DELETE",
            url: EventInit.getCtx()+"/flume/batchDeleteFlumesData",
            dataType:"json",
            contentType:"application/json", // 指定这个协议很重要
            data: JSON.stringify(rows),
            success: function (data) {
    ...
    ///////////////停止、重启、备份确定事件//////////////////////////////////////////
    $('#flumeModal').find('#sure').on('click', function () {
        var $modal = $('#flumeModal');
    ...
    function doMian(value) {
        if(!value)return
        var rows = $("#tb_flume").bootstrapTable('getSelections');
        var flumeId=rows[0].flumeId;
        var url="";
        var doWhat;
        switch(value)
        {
            case "back":
                url=EventInit.getCtx()+"/flume/backFlumes";
                doWhat="备份";
                break;
            case "reset":
                url=EventInit.getCtx()+"/flume/resetFlumes";
                doWhat="重启";
                break;
            case "stop":
                url=EventInit.getCtx()+"/flume/stopFlumes";
                doWhat="停止";
                break;
        }
        $('#loadingModal').modal('show');
