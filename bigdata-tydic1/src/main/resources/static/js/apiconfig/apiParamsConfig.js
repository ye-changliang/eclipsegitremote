...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_apis_params').bootstrapTable({
            url: EventInit.getCtx() + '/api-params-config/params',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
var ButtonInit = function () {
    ...
    oInit.Init = function () {
        ...
        $('#btn_query').on('click', function () {
            $('#tb_apis_params').bootstrapTable('refresh', {query: TableInit.queryParams});
        });
        ...
        $('#btn_update').on('click', function () {
            var rows = $("#tb_apis_params").bootstrapTable('getSelections');
            if (rows.length == 0) {
                toastr.info("请选择一条记录");
                return false;
            }
        ...
        $('#myModal').find("#save").on('click', function () {
            ...
            $.ajax({
                type: "POST",
                url: EventInit.getCtx() + "/api-params-config/save",
                data: $from.serialize(),
                success: function (data) {
                    if (data) {
                        toastr.success('保存成功！');
                        $('#myModal').modal('hide');
                        $('#tb_apis_params').bootstrapTable('refresh');
                    } else {
                        toastr.error('保存失败!');
                    }
        ...
        $('#delete-confirm').find('#delete-sure').on('click', function () {
            ...
            $.ajax({
                type: "DELETE",
                url: EventInit.getCtx() + "/api-params-config/delete",
                dataType: "json",
